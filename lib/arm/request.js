    ...
    _request(method, params) {
        ...
        return new Promise(function(resolve, reject) {
            return axios({
                ...
                data: params.data,
                headers: utils.object_update(payload.api_key || payload.auth_pass ? {
                    'Authorization': 'Basic '+
                        new Buffer((payload.api_key||'')+':'+(payload.auth_pass||''), 'binary')
                            .toString('base64')
                } : {}, self.constructor.default_headers || {} ),
                    ...
                    if (http_code == 200) {
                        ...
                        for (var error in exc) {
                            ...
                            if (exc[error].name && exc[error].name != obj.error_type)
                                continue;
                            throw new exc[error](obj.description, obj)
                        }

                        throw new exc.PayloadError(obj.description, obj)
                    }
